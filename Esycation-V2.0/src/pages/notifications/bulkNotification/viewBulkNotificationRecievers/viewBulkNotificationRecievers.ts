...
@IonicPage()
@Component({
  selector: "view-bulk-notification-reciever",
  templateUrl: "viewBulkNotificationRecievers.html"
})
export class ViewBulkNotificationsRecieverComponent extends BaseComponent {
  imagePath: String = ServerConfig.browseFilePath();
  staffs: Array<NotificationReciever> = new Array<NotificationReciever>();
  studentGuardians: Array<NotificationReciever> = new Array<
  ...
  ionViewDidLoad() {
    ...
    this.bulkNotificationService
      .findByNotificationId(notificationId, "STAFF", this.page, this.size)
      .subscribe(
        data => {
    ...
    this.bulkNotificationService
      .findByNotificationId(notificationId, "GUARDIAN", this.page, this.size)
      .subscribe(
        data => {
          if (data.contents.length == 0) {
