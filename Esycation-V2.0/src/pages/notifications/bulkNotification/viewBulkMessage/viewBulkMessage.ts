...
export class ViewBulkMessageComponent {
  ...
  constructor(
    private service: BulkNotificationService,
  ...
  ionViewDidLoad() {
    ...
    this.service.withSelections(id).subscribe(data => {
      this.notification = Object.assign({}, data);
      this.notification.iconTitle = this.findFirstLatter(
        this.notification.template.mode
      );
      this.notification.iconColor = this.findColor(
        this.notification.template.mode
      );
      this.comMode = this.notification.selections.receiverType;
  ...
  findColor(title: string): string {
    ...
    if (type == "S") return "#EA1E63";
    else if (type == "O") return "#0059B2";
    else if (type == "E") return "#8dc34b";
    else if (type == "P") return "#ff9800";
    else return "#9e9e9e";
  }
