...
export class ResultEntiryComponent extends BaseComponent {
  ...
  ionViewDidLoad() {
    ...
    this.resultEntryService.findByBatchResultId(id).subscribe(
      data => {
  ...
  onDraft() {
    if (this.validateMarks(this.resultDetails)) {
      ...
      this.resultEntryService.draft(this.resultDetails).subscribe(
        data => {
          console.log(data);
          this.commonServices.onDismissAll();
          this.commonServices.presentToast(
            "Result draft successfully",
            null,
            "success"
          );
  ...
  onPublish() {
    if (this.validateMarks(this.resultDetails)) {
      ...
      this.resultEntryService.publish(this.resultDetails).subscribe(
        data => {
          console.log(data);
          this.commonServices.onDismissAll();
          this.commonServices.presentToast(
            "Result publish successfully",
            null,
            "success"
          );
          this.navCtrl.setRoot("ResultEntiryViewComponent");
  ...
  onMarksValidate(marks: number, indexNo: number) {
    if (marks > this.resultDetails.marks) {
      this.resultDetails.studentResults[indexNo].error =
        "you can not enter marks out of " + this.resultDetails.marks;
    } else {
