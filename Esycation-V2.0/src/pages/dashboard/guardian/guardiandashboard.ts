...
export class GuardianDashboardComponent extends BaseComponent {
  ...
  initStudents() {
    this.studentObservable = Observable.create(observer => {
      this.studentService
        .findByGuardianIdsWithCourseAndBatch(this.session.findRemote())
        .subscribe(
          data => {
            if (data.contents) {
  ...
  initReportingTeacher(studentId: number) {
    this.reportingTeacherObservable = Observable.create(observer => {
      this.staffService.findClassTeacherByStudentId(studentId).subscribe(
        data => {
  ...
  initStudentApprovalRequests(studentId: number) {
    this.requestObservable = Observable.create(observer => {
      this.approvalService.findStudentApprovalRequests(studentId).subscribe(
        data => {
          if (data.contents && data.contents.length) {
  ...
  initExamWiseResult(studentId: number) {
    this.examWiseResultObservable = Observable.create(observer => {
      this.approvalService.findStudentResultStatistic(studentId).subscribe(
        data => {
          if (data.data && data.data.length) {
            ...
            this.examWiswResultOption = EchartOptionBuilder.build3DChart(
              tempData,
              chartConfiguration
            );
          } else {
  ...
  initTermWiseResult(studentId: number) {
    this.termWiseResultObservable = Observable.create(observer => {
      this.approvalService.findTermWiseResultStatistics(studentId).subscribe(
        data => {
          if (data.data && data.data.length) {
            var chartConfiguration = {
              yAxis: "Percentage ( % )",
            ...
            this.termWiswResultOption = EchartOptionBuilder.build3DChart(
              tempData,
              chartConfiguration
            );
          } else {
  ...
  initBatchTimeTable(studentId: number) {
    this.timeTableObservable = Observable.create(observer => {
      this.timetableService.findBatchTimetablesByStudentId(studentId).subscribe(
        data => {
          if (data) {
  ...
  initTodayTimetable(studentId: number) {
    this.todayTableObservable = Observable.create(observer => {
      this.timetableService
        .findTodayBatchTimetablesByStudentId(studentId)
        .subscribe(
          data => {
            if (data) {
