...
export class FileService extends BaseService<FileDetails>{
    ...
    uploadFile(data: any, folder: string): Observable<any> {
        ...
        let url: string = ServerConfig.getPath() + "/zuul/files/upload/" + folder + "/base64?overwrite=true"
    ...
    fileTransfer(file: File, folder: string): Observable<any> {
        ...
        let filePath: string = ServerConfig.getPath() + "/zuul/files/upload/" + folder + "/base64?overwrite=true"
